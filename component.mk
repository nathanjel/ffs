...
COMPONENT_EXTRA_CLEAN := $(COMPONENT_PATH)/include/ffs_files.h
COMPONENT_EXTRA_CLEAN += $(BUILD_DIR_BASE)/ffs-flash.map
COMPONENT_EXTRA_CLEAN += $(BUILD_DIR_BASE)/ffs.binary

ffs.o: $(COMPONENT_PATH)/include/ffs_files.h

$(BUILD_DIR_BASE)/ffs-flash.map: $(COMPONENT_PATH)/include/ffs_files.h

$(BUILD_DIR_BASE)/ffs.binary: $(COMPONENT_PATH)/include/ffs_files.h

$(COMPONENT_PATH)/include/ffs_files.h: $(PROJECT_PATH)/$(call dequote,$(CONFIG_FFS_FILES_FOLDER_PATH)) 
	$(PYTHON) $(COMPONENT_PATH)/build.py \
		$(IDF_PATH)/components/partition_table/ \
		$(PROJECT_PATH)/$(call dequote,$(CONFIG_FFS_FILES_FOLDER_PATH)) \
		$(COMPONENT_PATH)/include/ffs_files.h $(BUILD_DIR_BASE)/ffs-flash.map $(BUILD_DIR_BASE)/ffs.binary \
